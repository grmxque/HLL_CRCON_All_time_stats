...
that displays inner data on chat commands
...
def chat_commands(
    ...
    """
    - Punish ("!redeploy" or "!r" in chat)
    - Show player stats ("!me" in chat)
    """
    ...
    # Redeploy (punish)
    if struct_log["sub_content"] in ["!redeploy", "!r", "§R"]:
        try :
            rcon.punish(
    ...
    if struct_log["sub_content"] in ["!me"]:
        try:
            # Gather data
